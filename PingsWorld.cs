...
namespace Pings
{
	public class PingsWorld : ModWorld
	{
		...
		internal static void HighlightTile(int i, int j, ref Color drawColor)
		{
			...
			foreach (var ping in Pings)
			{
				if (ping.IsTile)
				{
					...
					if (tile.active() && tile.type == ping.Type && rect.Contains(point))
					{
